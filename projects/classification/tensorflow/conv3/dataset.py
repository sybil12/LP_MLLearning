...
class ImageData:
    ...
    def __init__(self, txt_file, batch_size, num_classes,
        ...
        data = data.map(self.parse_function)  # return img-处理后的图片(48*48), label_(one-hot编码的标签)
    ...
    def parse_function(self, filename, label):
        ...
        img = tf.image.random_flip_left_right(img)  # 以一定概率左右翻转图像
        # img = self.augment_dataset(img, self.image_size)
        img = self.augment_dataset(img)
        return img, label_
